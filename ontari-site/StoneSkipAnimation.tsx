...
type StoneSkipAnimationProps = {
  /** Number of skips to animate, 1–75. */
  skips: number;
  /** Bump this to re-trigger the animation with a new value. */
  playToken: number;
  onComplete?: () => void;
  className?: string;
};
...
/**
 * Procedurally animates a stone skipping `skips` times across a water line,
 * with each hop shorter and lower than the last (geometric decay), finishing
 * with a sink. Ripples are drawn in a thin, ink-line style to sit naturally
 * over the engraved wave video underneath.
 */
export function StoneSkipAnimation({
  ...
  useEffect(() => {
    ...
    // Geometric decay: each skip travels less distance and rises less high
    // than the previous one, the way a real stone loses energy on impact.
    ...
    function frame(now: number) {
      ...
      // Figure out which skip segment we're in.
      let t = elapsed;
      let segmentIndex = 0;
      let segX = startX;
      ...
      // Parabolic arc: 0 at start/end, peak at midpoint.
      const arc = Math.sin(Math.PI * progress);
      const y = waterY - arc * arcHeight - (1 - progress) * 0; // stays on water baseline at ends
      const startingDrop = segmentIndex === 0 ? (1 - Math.min(1, elapsed / 120)) * -20 : 0;

      // Spawn a ripple right as this hop begins (progress crosses 0 boundary).
      if (progress < 0.04) {
      ...
      if (!sinking) {
        ...
        // Sink: fade a bigger ripple at the final point, no stone drawn after.
